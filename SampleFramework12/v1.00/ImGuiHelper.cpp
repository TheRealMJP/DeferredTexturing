...
static ConstantBuffer<ImGuiConstants> CBuffer;
...
void Initialize(Window& window)
{
    ...
    VS = CompileFromFile(shaderPath.c_str(), "ImGuiVS", ShaderType::Vertex, ShaderProfile::SM51);
    PS = CompileFromFile(shaderPath.c_str(), "ImGuiPS", ShaderType::Pixel, ShaderProfile::SM51);
...
void EndFrame(ID3D12GraphicsCommandList* cmdList, D3D12_CPU_DESCRIPTOR_HANDLE rtv, uint32 displayWidth, uint32 displayHeight)
{
    ...
    // Setup orthographic projection matrix into our constant buffer
    {
        ...
        CBuffer.Data.ProjectionMatrix = Float4x4(Float4(2.0f/(R-L),   0.0f,           0.0f,       0.0f),
    ...
    CBuffer.SetAsGfxRootParameter(cmdList, 1);
    ...
    for(uint64 cmdListIdx = 0; cmdListIdx < drawData->CmdListsCount; cmdListIdx++)
    {
        ...
        for(int32 cmdIdx = 0; cmdIdx < drawList->CmdBuffer.size(); cmdIdx++)
        {
            ...
            else
            {
                ...
                D3D12_CPU_DESCRIPTOR_HANDLE psResources[1] = { texture->SRV.CPUHandle };
